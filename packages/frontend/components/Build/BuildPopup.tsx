...
const BuildPopup: React.FC<{
  isUpgradeInProgress: boolean;
  build: Build;
  onAction: (id: string) => void;
}> = memo(({ build, onAction, isUpgradeInProgress }) => {
  if (!build) return <></>;
  ...
  return (
    <>
      ...
      <br />
      <Button
        onClick={() => (isUpgradeInProgress ? null : onAction(build.id))}
        className={styles.onUpgrade}
        color="gradient"
        disabled={isUpgradeInProgress || isLastLevel}
      >
        {isUpgradeInProgress
          ? "Upgrading..."
          : isLastLevel
