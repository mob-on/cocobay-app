...
const calculateFontSize = (tapCount: number) => {
  ...
  const maxDigits = 18;
  const fontSize = (maxFontSize - minFontSize) * (maxDigits - digits) / (maxDigits - 6) + minFontSize;
  return `${fontSize}px`;
}
...
const TapCounter: React.FC = () => {
  ...
  return <div className={styles.tapCounter}>
    <h1 style={{ fontSize: calculateFontSize(data.tapCount) }}>{ formattedTapCount }</h1>
  </div>;
};
